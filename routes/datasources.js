...
router.get('/defaults', (req, res) => {
  res.json({
    success: true,
    data: {
      postgres: { host: process.env.DB_HOST || 'postgres', port: 5432, database: process.env.DB_NAME || 'dna', username: process.env.DB_USER || 'postgres' },
      mysql: { host: 'localhost', port: 3306, database: 'mysql' },
...
router.post('/test', async (req, res) => {
  ...
  try {
    const body = normalizeSourceBody(req.body);
    await dbConnector.addSource({ ...body, id: testId });
    await dbConnector.removeSource(testId);
    res.json({ success: true, message: 'Connection successful' });
  } catch (err) {
    await dbConnector.removeSource(testId).catch(() => {});
...
router.post('/:id/monitor', async (req, res) => {
  try {
    ...
    for (const cfg of configsToRun) {
      ...
      const jobId = autoMonitor.startJob(sourceId, cfg, { pollIntervalMs, batchSize });
      jobIds.push(jobId);
    }
...
function normalizeSourceBody(body) {
  return {
    ...
    port: parseInt(body.port, 10) || defaultPort(body.type),
